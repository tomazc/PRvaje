...
web-sl/%.ipynb: %.ipynb
	...
	jupyter nbconvert --execute --ExecutePreprocessor.timeout=180 --allow-errors --to notebook $(addprefix web-sl/, $(notdir $^)) --inplace
...
web-en/%.ipynb: %.ipynb
	...
	jupyter nbconvert --execute --ExecutePreprocessor.timeout=180 --allow-errors --to notebook $(addprefix web-en/, $(notdir $^)) --inplace
...
pdf/PR.pdf: biblio.bib tex-sl/PR.tex tex-sl/glava.tex $(NOTEBOOKS_res_tex-sl) $(NOTEBOOKS_tex-sl) $(NOTEBOOKS_nal_tex-sl)
	rm -f pdf/PR.pdf && \
	rm -f tex-sl/PR.pdf && \
	cd tex-sl && \
	xelatex PR && \
...
pdf/DM.pdf: biblio.bib tex-en/DM.tex tex-en/heading.tex $(NOTEBOOKS_res_tex-en) $(NOTEBOOKS_tex-en) $(NOTEBOOKS_nal_tex-en)
	rm -f pdf/DM.pdf && \
	rm -f tex-en/DM.pdf && \
	cd tex-en && \
	xelatex DM && \
